...
def generate_launch_description():
    ...
    declare_world = DeclareLaunchArgument(
        "world", default_value="empty.sdf", description="World file to load in Gazebo"
    )
    ...
    declare_path_type = DeclareLaunchArgument(
        ...
        description=(
            "Path type: hover, circle, infinity, infinity3d, infinity3dmod, "
            "lissajous3d, helix3d, poly3d, takeoff_land."
        ),
    )

    declare_path_scale = DeclareLaunchArgument(
        "path_scale", default_value="5.0", description="Path scaling factor"
    )
    ...
    declare_Kd = DeclareLaunchArgument(
        "Kd",
        default_value="[2.05,2.05,2.05,2.05,2.05,2.05]",
    ...
    spawn_launch_file = os.path.join(
        hex_pkg, "launch", "spawn_robot_adaptive.launch.py"
    )
    ...
    control_allocation_params = all_params.get("control_allocation_node", {}).get(
        "ros__parameters", {}
    )
    control_allocation_params["allocation_method"] = LaunchConfiguration(
        "allocation_method"
    )
