...
class AdaptivePathFollowingNode(Node):
    def __init__(self):
        ...
        self.timer = self.create_timer(
            0.01, self.publish_wrench, callback_group=cb_group
        )  # 100 Hz
    ...
    def publish_wrench(self):
        ...
        if elapsed > self.pick_up_waiting_time:
            ...
            try:
                # Update desired path
                self.H_des, self.V_des, self.A_des = self.path_generator.generate(
                    ctrl_time
                )
                ...
                if self.controller_type == "Adaptive":
                    ...
                    self.get_logger().info(
                        f"Estimated Mass: {m}. Estimated CoG: {cog[0]}, {cog[1]}, {cog[2]}."
                    )
                ...
                wrench_msg.torque.x, wrench_msg.torque.y, wrench_msg.torque.z = wrench[
                    0:3
                ]
