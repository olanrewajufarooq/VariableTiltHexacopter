...
def generate_launch_description():
    ...
    declare_world = DeclareLaunchArgument('world', default_value='industrial_warehouse.sdf', description='World file to load in Gazebo')
    declare_path_type = DeclareLaunchArgument('path', default_value='square', description='Path type: circle, square, infinity_loop, takeoff_land, hover.')
    declare_path_scale = DeclareLaunchArgument('path_scale', default_value='3.0', description='Path scaling factor')
    declare_path_period = DeclareLaunchArgument('path_period', default_value='60.0', description='Traversal speed along the path')
